...
class ShippedPdf extends Component {
    constructor(props) {
        super(props);
        this.state = {
            status: []
    ...
    printDocument() {
        const input = document.getElementById('pdfdiv4');
        html2canvas(input)
            .then((canvas) => {
                var imgWidth = 200;
                var pageHeight = 290;
    ...
    render() {
        return (
            <div>
                <AppBar />
                <NavBar />
                <div id="pdfdiv4" style={{ maxWidth: '100%', background: '#ffffff', marginBottom: "30px", minHeight: '1300px' }}>
                        ...
                        <div className="row">
                            <div>
                                <div className="d-flex p-2" style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '200px', position: 'relative', marginBottom: '-50px', marginTop: '20px' }}>
                                    <h4 tag='div' className='display-1 pb-3 mb-3 border-bottom' style={{ fontWeight: "bold", fontSize: '180%' }}>SHIPPED ORDERS</h4>
                                </div>
