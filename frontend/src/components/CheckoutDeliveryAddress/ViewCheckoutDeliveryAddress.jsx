...
class ViewCheckoutDeliveryAddress extends Component {
    constructor(props) {
        super(props);
    ...
    updateDeliveryAddress(id, name, address1, address2, address3, phone, email) {
        ...
        window.location.href = APP_ROUTES.USER_CHECKOUT_UPDATE_ADDRESS;
    }
    ...
    render() {
        return (
            <div>
                ...
                <div className="d-flex p-2" style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '200px', marginTop: '20px' }}>
                        ...
                        <div>
                            {
                                (this.state.deliveryAddress.length > 0 ?
                                    ...
                                    :
                                    <button type="button" className="btn btn-info" style={{ float: 'right', padding: '12px 28px', marginBottom: '30px' }} onClick={() => { window.location.href = APP_ROUTES.USER_CHECKOUT_ADD_ADDRESS }}>
                                        <PlusLg /> Add new Delivery Address
                                    </button>
                                ...
                                {this.state.deliveryAddress.map((item) => (
                                    <form style={{ paddingTop: "70px", paddingLeft: "10%" }}>
                                        ...
                                        <div className="btn-toolbar justify-content-between" role="toolbar" aria-label="Toolbar with button groups">
                                            <button type="button" className="btn btn-outline-primary" style={{ float: 'right', padding: '12px 68px', marginTop: '20px', fontWeight: 'bold', fontSize: "130%" }} onClick={() => this.updateDeliveryAddress(item._id, item.name, item.address1, item.address2, item.address3, item.phone, item.email)}><Pencil /> Update</button>
                                            <button type="button" className="btn btn-outline-success" style={{ float: 'right', padding: '12px 38px', marginTop: '17px', fontWeight: 'bold', fontSize: "130%", marginRight:'100px' }} onClick={() => { window.location.href = APP_ROUTES.USER_CHECKOUT }}>Continue to Checkout</button>
                                        </div>
                                    </form>
                                ))}
                            </div>
...
export default ViewCheckoutDeliveryAddress;